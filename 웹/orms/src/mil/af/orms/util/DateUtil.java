...
public class DateUtil {
	...
	/**
	 * 해당 년월의 일수를 구해줌 ex)1월:getLastDay('2011','1')
	 * @param year
	 * @param month
	 * @return
	 */
	 ...
	 * @return
	 */
	public static String getBeforeOrNextMonth(int year,int month,int plusMinus,String dateForm){
		return getDateString(getCalendar(year,month-1+plusMinus),"YYYYMM");
	}
	
	/**
	 * 이전날 or 다음날을 구해줌, plusMinus : 0 -> 오늘 리턴 , plusMinus : 1 -> 하루뒤
	 * plusMinus:2 -> 2일뒤, plusMinus: -1 -> 하루 전 ....
	 * @param year
	 * @param month
	 * @param day
	 * @param plusMinus
	 ...
	 * @return
	 */
	public static String getBeforeOrNextDay(int year,int month,int day,int plusMinus,String dateForm){
	...
	public static List<RangeVO> getWeekRangeList(int year,int month){
		List<RangeVO> resultList=new ArrayList<RangeVO>();
		int day=1; 
		
		Calendar calendar=getCalendar(year,month-1,day);
		int maxDay=calendar.getActualMaximum(Calendar.DAY_OF_MONTH);
		int weekNumber=1;
		int dayWeek=calendar.get(Calendar.DAY_OF_WEEK); // 달 첫날의 요일
		// 목,금,토,일 중 한개면 이전달의 x주, 월,화,수 일 시엔 이번달 1주
		int dayToMonday=(9-dayWeek)%7;
		int mondayDate=day+dayToMonday;
		if(dayWeek<5&&dayWeek>1){ //월(2),화(3),수(4) 일땐  첫주로 넣어주고 다음주 시작
			String[] thisWeek=getThisWeek(year, month, day, "YYYYMMDD", 0, 1);
			RangeVO rangeVO=new RangeVO(month+"월 "+weekNumber+"주",Integer.parseInt(thisWeek[0]),Integer.parseInt(thisWeek[1]));
		...
		int whileCount=0;
		while(mondayDate<=maxDay){
			if(maxDay-mondayDate>1){
				String[] thisWeek=getThisWeek(year, month, mondayDate, "YYYYMMDD", 0, 1);
				RangeVO rangeVO=new RangeVO(month+"월 "+weekNumber+"주",Integer.parseInt(thisWeek[0]),Integer.parseInt(thisWeek[1]));
				resultList.add(rangeVO);
				mondayDate+=7;
				weekNumber++;
	...
	public static final void main(String[] args){
		System.out.println(getWeekRangeList(2012,2).toString());
	}

	/**
	 * 시작일 끝일 사이에 포함되는지 체크함 YYYYMMDD형식으로 넘겨야함
	 * @param date
	 * @param startDate
	 * @param endDate
	 * @return
	 */
	public static boolean isBetweenDay(String date, Integer startDate,
			Integer endDate) {
