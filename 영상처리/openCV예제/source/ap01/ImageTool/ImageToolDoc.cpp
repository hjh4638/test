...
BEGIN_MESSAGE_MAP(CImageToolDoc, CDocument)
	...
	ON_COMMAND(ID_BINARIZATION_ITERATIVE, &CImageToolDoc::OnBinarizationIterative)
	...
	ON_COMMAND(ID_OPENCV_TEST, &CImageToolDoc::OnOpencvTest)
END_MESSAGE_MAP()
...
void CImageToolDoc::OnBinarizationIterative()
{
	CDib dib = m_Dib;
...
void CImageToolDoc::OnInvariantMoments()
{
	...
	CString str = _T("Invariant moments: (x0.000001)\n\n");
	for( int i = 0 ; i < 7 ; i++ )
		str.AppendFormat(_T("m[%d] = %16.10lf\n"), i, m[i]*1000000);

	AfxMessageBox(str);
}

void CImageToolDoc::OnOpencvTest()
{
	IplImage* image = NULL;

	// CDib -> IplImage

	CDib_TO_IplImage(&m_Dib, &image);
	cvNamedWindow("OpenCV Test", CV_WINDOW_AUTOSIZE);
	cvShowImage("OpenCV Test", image);

	// IplImage -> CDib

	CDib dib;
	IplImage_TO_CDib(image, &dib);
	AfxNewImage(dib);

	// 결과 메시지 출력

	CString str;
	str.Format(_T("OpenCV 변환 테스트가 성공했습니다.\n"));
	str.AppendFormat(_T("확인 버튼을 누르면 OpenCV Test 창이 사라집니다."));
	AfxMessageBox(str);

	// OpenCV 라이브러리에서 사용한 메모리 해제
