...
public:
	...
	BigDecimal(unsigned char* str, int size){
		this->digit = new unsigned char[size+1];
		for (int i = 0; i < size; i++){
			//str이 숫자로 이루어져 있을때
			//digit[i] = str[size - i - 1] - 48;
			//str이 영어로 되있을때
			digit[i] = str[size - i - 1]-97;
		}
		digit[size] = 0;
		this->size = size;
		this->sign = false;
	}
	BigDecimal(char* in, int start, int last){
		int size = last - start + 1;
		this->digit = new unsigned char[size+1];
		for (int i = 0; i < size; i++){
			//str이 숫자로 이루어져 있을때
			//digit[i] = in[last-i] - 48;
			digit[i] = in[last - i] -97;
		}
		digit[size] = 0;
		this->size = size;
		this->sign = false;
	...
	void printDecimal(){
		...
		for (; i >= 0; i--){
			...
			unsigned char num = this->digit[i] + 97;
			cout << num;
		}
		cout << endl;
	}
	...
	//제곱 구해주는 함수
	unsigned long long getSqure(int opernand, int num){
	...
	unsigned long long getInt(){
		...
		for (int i = 0; i < this->size; i++){
			result += this->digit[i] * getSqure(10, i);
		}
		return result;
	}
	//꺼꾸로 연산됨
	...
	BigDecimal modular(BigDecimal* A, BigDecimal* B){
		...
		result.digit += (result.size - B->size);
		result.size = B->size;

		i = (A->size - B->size);
		for (; i >= 0; i--){
			while (result > (*B)){
				MinusForDivide(&result, B);
			}
			result.digit--;
